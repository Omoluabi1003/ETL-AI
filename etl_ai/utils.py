...
def copy_env_file() -> None:
    """
    Copy the .env file from the etl_ai directory to the sandbox directory.
    """
    project_env = Path("etl_ai/.env")
    sandbox_env = Path("sandbox/.env")
    
    if not project_env.exists():
        print("\nWarning: .env file not found in etl_ai directory")
        print(f"  Looked for: {project_env.absolute()}")
        return
    ...
    try:
        ...
        # Copy the .env file
        shutil.copy2(project_env, sandbox_env)
...
def setup_sandbox() -> None:
    ...
    # Copy .env file from etl_ai to sandbox
    copy_env_file()
